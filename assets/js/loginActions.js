 ...
 * @returns True if follows the given pattern otherwise false.
 */
...
/**
 * Function yo both validate and post a message to the server.
 * If it gets a positive response from the server, it will redirect to /calendar.
 * Otherwise a message will show on the bottom of the form.
 * @param {form} The form that needs checking.
 */
async function loginValidation(form) {
  if (
    verifyPasswordInput(form.password.value) &&
    ...
    try {
      ...
      if (
        ...
        showPassHint("E-mail eller adgangskode forkert! Prøv igen...");
      } else {
